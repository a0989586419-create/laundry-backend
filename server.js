...
function calculateBonusPoints(amount) {
  if (amount >= 1000) return Math.floor(amount * 0.12); // 12% 回饋
  if (amount >= 500) return Math.floor(amount * 0.10);  // 10% 回饋
  if (amount >= 300) return Math.floor(amount * 0.05);  // 5% 回饋
  return 0;
}

async function applyTopupBonus(userId, groupId, amount) {
  const bonus = calculateBonusPoints(amount);
  ...
  try {
    ...
    `, [userId, groupId, bonus]);
    // Record bonus transaction
    await db.query(`
      INSERT INTO transactions (line_user_id, group_id, type, amount, description, created_at)
      VALUES ($1, $2, 'topup_bonus', $3, $4, NOW())
    `, [userId, groupId, bonus, `儲值 ${amount} 贈送 ${bonus} 點 (${amount >= 1000 ? '12%' : amount >= 500 ? '10%' : '5%'} 回饋)`]);
    console.log(`[Bonus] User ${userId} topup ${amount}, bonus ${bonus} (group: ${groupId})`);
    return { bonus };
  } catch (e) {
    console.error('[Bonus] Apply bonus error:', e.message);
    return { bonus: 0 };
  }
}
  ...
  return {
    ...
    body: {
      ...
      contents: [
        ...
        { type: 'separator', color: '#1A1A1E', margin: 'md' },
        ...
        { type: 'separator', color: '#1A1A1E', margin: 'md' },
        { type: 'box', layout: 'horizontal', margin: 'md', paddingAll: '10px', backgroundColor: '#0A0A0F', cornerRadius: '8px', contents: [
  ...
  return {
    ...
    body: {
      ...
      contents: [
        ...
        { type: 'box', layout: 'horizontal', margin: 'md', paddingAll: '12px', backgroundColor: '#0A0A0F', cornerRadius: '8px', contents: [
...
function buildPromoFlexMessage(message, options = {}) {
  ...
  const bubble = {
    ...
    body: {
      type: 'box', layout: 'vertical', paddingAll: '20px', spacing: 'md',
      contents: [
        { type: 'text', text: title || '雲管家通知', size: 'lg', weight: 'bold', color: '#E5B94C' },
        { type: 'separator', color: '#1A1A1E', margin: 'lg' },
        { type: 'text', text: message, wrap: true, size: 'md', color: '#E0E0E0', margin: 'md' },
      ]
    },
    footer: {
      type: 'box', layout: 'vertical', paddingAll: '16px', spacing: 'sm',
...
app.post('/api/machines/state', async (req, res) => {
  try {
    ...
    // Check previous state before update (for wash-complete notification)
    const prevStateRow = (await db.query('SELECT state FROM machine_current_state WHERE machine_id=$1', [machineId])).rows[0];
    const prevState = prevStateRow?.state;
    ...
    `, [machineId, newState, remainSec || 0]);

    // [Notify] Send wash-complete notification when state transitions from running to done/idle/completed
    if (prevState === 'running' && (newState === 'done' || newState === 'idle' || newState === 'completed')) {
      try {
        const runningOrders = await db.query(
          `SELECT o.id, o.store_id, mb.line_user_id, s.name as store_name, m.name as machine_name, s.group_id
           FROM orders o
        ...
        // Update orders to done
        await db.query(`UPDATE orders SET status='done', completed_at=NOW() WHERE machine_id=$1 AND status='running'`, [machineId]);
        // Lookup support info
        let supportUrl = '', supportPhone = '';
        if (runningOrders.rows.length > 0 && runningOrders.rows[0].group_id) {
          const sgRes = await db.query('SELECT support_url, support_phone FROM store_groups WHERE id=$1', [runningOrders.rows[0].group_id]);
        ...
        for (const ord of runningOrders.rows) {
          console.log(`[Notify] Wash complete: ${ord.store_name} ${ord.machine_name} -> user ${ord.line_user_id}`);
          sendLineFlexMessage(ord.line_user_id, '洗衣完成通知', buildCompleteFlexMessage(ord.store_name, ord.machine_name, supportUrl, supportPhone, ord.group_id, ord.store_id), { pushType: 'auto_complete', storeId: ord.store_id, description: `洗衣完成 ${ord.store_name} ${ord.machine_name}` }).catch(e => console.error('[Notify] Push error:', e.message));
        }
      } catch (notifyErr) {
        console.error('[Notify] State change notification error:', notifyErr.message);
      }
    }
...
app.get('/api/topup/confirm', async (req, res) => {
  try {
    ...
    if (result.returnCode === '0000') {
      ...
      // [Bonus] Apply topup bonus
      const { bonus } = await applyTopupBonus(userId, groupId, amountInt);
      // LINE Push: topup success (with bonus info)
      ...
      if (bonus > 0) {
        sendLinePush(userId, [buildTopupSuccessNotification({ amount: amountInt, bonus, newBalance: walletRow?.balance || amountInt })], { pushType: 'auto_topup', groupId, description: `LINE Pay儲值成功 ${grName} NT$${amountInt} 贈${bonus}點` }).catch(() => {});
      } else {
      ...
      res.redirect(`${FRONTEND_URL}/?status=topup_success&amount=${amountInt}&bonus=${bonus}`);
    } else {
...
app.post('/api/topup/jkopay/result', async (req, res) => {
  try {
    ...
    if (status === 'SUCCESS') {
      ...
      if (bonus > 0) {
        sendLinePush(userId, [buildTopupSuccessNotification({ amount: amountInt, bonus, newBalance: walletRow?.balance || amountInt })], { pushType: 'auto_topup', groupId, description: `街口支付儲值成功 ${grName} NT$${amountInt} 贈${bonus}點` }).catch(() => {});
      } else {
      ...
      console.log(`[JKOPay] Topup success: orderId=${platform_order_id}, amount=${amountInt}, bonus=${bonus}`);
    } else {
      await db.query(`UPDATE orders SET status='failed', jkopay_order_id=$2 WHERE id=$1`, [platform_order_id, order_id]);
  ...
  return {
    ...
    contents: {
      ...
      header: {
        ...
        contents: [
          {
            type: 'box', layout: 'vertical', backgroundColor: '#111115',
    ...
    contents: {
      ...
      header: {
        ...
        contents: [
          ...
          { type: 'text', text: '全台 5 間門市 ∙ 30+ 台設備 ∙ 24H 為您服務', color: BRAND_GOLD, size: 'xs', margin: 'sm' }
        ]
      },
      body: {
        type: 'box', layout: 'vertical', spacing: 'lg', paddingAll: '20px',
    ...
    contents: {
      ...
      body: {
        ...
        contents: [
          ...
          { type: 'text', text: '🏪 全台 5 間合作門市 ∙ 30+ 台設備管理 ∙ 7 天快速導入', size: 'xs', color: BRAND_GOLD, margin: 'sm', wrap: true },
          {
            type: 'box', layout: 'horizontal', margin: 'lg', contents: [
              { type: 'box', layout: 'vertical', backgroundColor: '#151518', cornerRadius: '8px', width: '36px', height: '36px', justifyContent: 'center', alignItems: 'center', flex: 0, contents: [
...
// ---- Push Notification Flex Builders ----

function buildPaymentSuccessNotification({ storeName, machineName, mode, amount, paymentMethod }) {
  const now = new Date().toLocaleString('zh-TW', { timeZone: 'Asia/Taipei' });
  return {
    type: 'flex', altText: '付款成功通知',
    contents: {
      type: 'bubble', size: 'mega',
      styles: {
        header: { backgroundColor: '#0A0A0F' },
      ...
      header: {
        type: 'box', layout: 'horizontal', paddingAll: '20px', spacing: 'md',
        contents: [
          { type: 'box', layout: 'vertical', width: '40px', height: '40px', backgroundColor: '#151518', cornerRadius: '20px', justifyContent: 'center', alignItems: 'center', contents: [
            { type: 'text', text: '\u2705', size: 'lg', align: 'center' }
          ]},
          { type: 'box', layout: 'vertical', justifyContent: 'center', contents: [
            { type: 'text', text: '\u2705 付款成功', color: '#4ADE80', weight: 'bold', size: 'lg' },
            { type: 'text', text: '交易已完成', color: '#999999', size: 'xs' }
          ]}
        ]
      },
      body: {
        ...
        contents: [
          { type: 'box', layout: 'horizontal', contents: [
            { type: 'text', text: '門市名稱', size: 'sm', color: '#999999', flex: 3 },
            { type: 'text', text: storeName || '---', size: 'sm', color: '#FFFFFF', flex: 5, align: 'end', weight: 'bold' }
          ]},
          { type: 'box', layout: 'horizontal', contents: [
            { type: 'text', text: '機器編號', size: 'sm', color: '#999999', flex: 3 },
            { type: 'text', text: machineName || '---', size: 'sm', color: '#FFFFFF', flex: 5, align: 'end' }
          ]},
          { type: 'box', layout: 'horizontal', contents: [
            { type: 'text', text: '洗衣模式', size: 'sm', color: '#999999', flex: 3 },
            { type: 'text', text: mode || '---', size: 'sm', color: '#FFFFFF', flex: 5, align: 'end' }
          ]},
          { type: 'separator', color: '#1A1A1E', margin: 'md' },
          { type: 'box', layout: 'horizontal', margin: 'md', contents: [
            { type: 'text', text: '付款金額', size: 'md', color: '#FFFFFF', flex: 3, weight: 'bold' },
            { type: 'text', text: `NT$ ${amount || 0}`, size: 'lg', color: '#E5B94C', flex: 5, align: 'end', weight: 'bold' }
          ]},
          { type: 'separator', color: '#1A1A1E', margin: 'md' },
          { type: 'box', layout: 'horizontal', contents: [
            { type: 'text', text: '付款方式', size: 'xs', color: '#999999', flex: 3 },
            { type: 'text', text: paymentMethod || '錢包付款', size: 'xs', color: '#999999', flex: 5, align: 'end' }
          ]},
          { type: 'box', layout: 'horizontal', contents: [
            { type: 'text', text: '付款時間', size: 'xs', color: '#999999', flex: 3 },
            { type: 'text', text: now, size: 'xs', color: '#999999', flex: 5, align: 'end' }
          ]}
        ]
      },
      footer: {
        type: 'box', layout: 'vertical', spacing: 'sm', paddingAll: '15px',
        contents: [
          { type: 'button', style: 'primary', color: '#E5B94C', height: 'sm', action: { type: 'uri', label: '查看機器狀態', uri: LIFF_WASH } }
        ]
      }
    }
  };
...
function buildAlmostDoneNotification({ storeName, machineName, remainingMinutes }) {
  return {
    type: 'flex', altText: `${machineName || '機器'} 還有 ${remainingMinutes || 5} 分鐘完成`,
    contents: {
      type: 'bubble', size: 'mega',
      styles: {
        header: { backgroundColor: '#0A0A0F' },
      ...
      header: {
        ...
        contents: [
          ...
          { type: 'box', layout: 'vertical', justifyContent: 'center', contents: [
            { type: 'text', text: '\u23F0 即將完成', color: '#F59E0B', weight: 'bold', size: 'lg' },
            { type: 'text', text: '請準備前往取衣', color: '#999999', size: 'xs' }
          ]}
        ]
      },
      body: {
        type: 'box', layout: 'vertical', spacing: 'md', paddingAll: '20px',
        contents: [
          { type: 'text', text: `您在 ${storeName || '門市'} 的 ${machineName || '機器'} 還有 ${remainingMinutes || 5} 分鐘即可完成`, size: 'md', wrap: true, color: '#FFFFFF' },
          { type: 'box', layout: 'horizontal', margin: 'lg', paddingAll: '12px', backgroundColor: '#0A0A0F', cornerRadius: '8px', contents: [
            { type: 'text', text: '\u23F1 剩餘時間', size: 'sm', color: '#E5B94C', flex: 2, weight: 'bold' },
            { type: 'text', text: `${remainingMinutes || 5} 分鐘`, size: 'lg', color: '#F59E0B', flex: 3, align: 'end', weight: 'bold' }
          ]},
          { type: 'separator', color: '#1A1A1E', margin: 'md' },
          { type: 'text', text: '請準備前往取衣，避免佔用機器', size: 'xs', color: '#999999', margin: 'md', wrap: true }
        ]
      },
      footer: {
        type: 'box', layout: 'vertical', spacing: 'sm', paddingAll: '15px',
        contents: [
          { type: 'button', style: 'primary', color: '#E5B94C', height: 'sm', action: { type: 'uri', label: '導航前往', uri: LIFF_WASH } }
        ]
      }
    }
  };
...
function buildWashCompleteNotification({ storeName, machineName, totalTime }) {
  return {
    type: 'flex', altText: `${machineName || '機器'} 洗衣完成！`,
    contents: {
      type: 'bubble', size: 'mega',
      styles: {
        header: { backgroundColor: '#0A0A0F' },
      ...
      header: {
        ...
        contents: [
          ...
          { type: 'box', layout: 'vertical', justifyContent: 'center', contents: [
            { type: 'text', text: '\uD83C\uDF89 洗衣完成', color: '#4ADE80', weight: 'bold', size: 'lg' },
            { type: 'text', text: '請盡快前往取衣', color: '#999999', size: 'xs' }
          ]}
        ]
      },
      body: {
        type: 'box', layout: 'vertical', spacing: 'md', paddingAll: '20px',
        contents: [
          { type: 'text', text: `您在 ${storeName || '門市'} 的 ${machineName || '機器'} 已完成洗衣！`, size: 'md', wrap: true, color: '#FFFFFF', weight: 'bold' },
          { type: 'separator', color: '#1A1A1E', margin: 'md' },
          { type: 'box', layout: 'horizontal', margin: 'md', contents: [
            { type: 'text', text: '門市', size: 'sm', color: '#999999', flex: 2 },
            { type: 'text', text: storeName || '---', size: 'sm', color: '#FFFFFF', flex: 3, align: 'end', weight: 'bold' }
          ]},
          { type: 'box', layout: 'horizontal', contents: [
            { type: 'text', text: '機器', size: 'sm', color: '#999999', flex: 2 },
            { type: 'text', text: machineName || '---', size: 'sm', color: '#FFFFFF', flex: 3, align: 'end' }
          ]},
          ...(totalTime ? [{ type: 'box', layout: 'horizontal', contents: [
            { type: 'text', text: '總耗時', size: 'sm', color: '#999999', flex: 2 },
            { type: 'text', text: `${totalTime} 分鐘`, size: 'sm', color: '#FFFFFF', flex: 3, align: 'end' }
          ]}] : []),
          { type: 'separator', color: '#1A1A1E', margin: 'md' },
          { type: 'box', layout: 'horizontal', margin: 'md', paddingAll: '12px', backgroundColor: '#0A0A0F', cornerRadius: '8px', contents: [
            { type: 'text', text: '請盡快取衣，超過 30 分鐘將自動佔機計費', size: 'xs', color: '#F59E0B', wrap: true, weight: 'bold' }
          ]}
        ]
      },
      footer: {
        ...
        contents: [
          { type: 'button', style: 'primary', color: '#E5B94C', height: 'sm', action: { type: 'uri', label: '導航前往', uri: LIFF_WASH } },
          { type: 'button', style: 'link', color: '#E5B94C', height: 'sm', action: { type: 'uri', label: '查看機器狀態', uri: LIFF_WASH } }
        ]
      }
    }
  };
...
function buildTopupSuccessNotification({ amount, bonus, newBalance }) {
  const now = new Date().toLocaleString('zh-TW', { timeZone: 'Asia/Taipei' });
  const bodyContents = [
    { type: 'box', layout: 'horizontal', contents: [
      { type: 'text', text: '儲值金額', size: 'md', color: '#FFFFFF', flex: 3 },
      { type: 'text', text: `+NT$ ${amount || 0}`, size: 'lg', color: '#4ADE80', flex: 5, align: 'end', weight: 'bold' }
    ]}
  ];
  if (bonus) {
    bodyContents.push(
      { type: 'box', layout: 'horizontal', margin: 'sm', contents: [
        { type: 'text', text: '加碼回饋', size: 'sm', color: '#F59E0B', flex: 3 },
        { type: 'text', text: `+NT$ ${bonus}`, size: 'sm', color: '#F59E0B', flex: 5, align: 'end', weight: 'bold' }
      ]}
    );
  }
  bodyContents.push(
    { type: 'separator', color: '#1A1A1E', margin: 'md' },
    { type: 'box', layout: 'horizontal', margin: 'md', contents: [
      { type: 'text', text: '目前餘額', size: 'md', color: '#FFFFFF', flex: 3, weight: 'bold' },
      { type: 'text', text: `NT$ ${newBalance || 0}`, size: 'lg', color: '#E5B94C', flex: 5, align: 'end', weight: 'bold' }
    ]},
    { type: 'separator', color: '#1A1A1E', margin: 'md' },
    { type: 'box', layout: 'horizontal', margin: 'sm', contents: [
      { type: 'text', text: '儲值時間', size: 'xs', color: '#999999', flex: 3 },
      { type: 'text', text: now, size: 'xs', color: '#999999', flex: 5, align: 'end' }
    ]}
  );

  return {
    type: 'flex', altText: `儲值成功 NT$ ${amount || 0}`,
    contents: {
      type: 'bubble', size: 'mega',
      styles: {
        header: { backgroundColor: '#0A0A0F' },
      ...
      header: {
        ...
        contents: [
          ...
          { type: 'box', layout: 'vertical', justifyContent: 'center', contents: [
            { type: 'text', text: '\uD83D\uDCB0 儲值成功', color: '#4ADE80', weight: 'bold', size: 'lg' },
            { type: 'text', text: '餘額已更新', color: '#999999', size: 'xs' }
          ]}
        ]
      },
      body: {
      ...
      footer: {
        type: 'box', layout: 'vertical', spacing: 'sm', paddingAll: '15px',
        contents: [
          { type: 'button', style: 'primary', color: '#E5B94C', height: 'sm', action: { type: 'uri', label: '查看餘額', uri: LIFF_PROFILE } }
        ]
      }
    }
  };
  ...
  return {
    ...
    contents: {
      ...
      header: {
        type: 'box', layout: 'vertical', paddingAll: '20px',
        contents: [
          { type: 'text', text: '🎉 最新優惠活動', color: '#E5B94C', weight: 'bold', size: 'lg' }
        ]
      },
      body: {
        type: 'box', layout: 'vertical', spacing: 'md', paddingAll: '20px',
        contents: [
          ...
          },
          {
            type: 'box', layout: 'vertical', backgroundColor: '#0A0A0F', cornerRadius: '8px', paddingAll: '12px', margin: 'md',
            contents: [
              { type: 'text', text: '🎁 推薦好友', size: 'sm', weight: 'bold', color: '#7B7BFF' },
              { type: 'text', text: '推薦朋友加入，雙方各得 NT$10 洗衣金', size: 'xs', wrap: true, margin: 'sm', color: '#E0E0E0' }
            ]
          }
        ]
      },
      footer: {
        type: 'box', layout: 'vertical', spacing: 'sm', paddingAll: '15px',
        contents: [
          { type: 'button', style: 'primary', color: '#E5B94C', action: { type: 'uri', label: '馬上去洗衣', uri: LIFF_URL } }
        ]
      }
    }
  };
  ...
  return {
    ...
    contents: {
      ...
      header: {
        type: 'box', layout: 'vertical', paddingAll: '20px',
        contents: [
          { type: 'text', text: '🔥 烘衣小指南', color: '#E5B94C', weight: 'bold', size: 'lg' }
        ]
      },
      body: {
        type: 'box', layout: 'vertical', spacing: 'md', paddingAll: '20px',
        contents: [
          { type: 'text', text: '溫度選擇建議', size: 'md', weight: 'bold', color: '#FFFFFF' },
          { type: 'separator', color: '#1A1A1E', margin: 'lg' },
          { type: 'box', layout: 'horizontal', margin: 'lg', contents: [
            { type: 'text', text: '低溫 40°C', size: 'sm', flex: 4, color: '#FFFFFF' },
            { type: 'text', text: '內衣褲、絲質衣物', size: 'sm', flex: 6, wrap: true, color: '#E0E0E0' }
          ]},
          { type: 'box', layout: 'horizontal', margin: 'sm', contents: [
            { type: 'text', text: '中溫 60°C', size: 'sm', flex: 4, color: '#FFFFFF' },
            { type: 'text', text: '一般衣物、T恤、褲子', size: 'sm', flex: 6, wrap: true, color: '#E0E0E0' }
          ]},
          { type: 'box', layout: 'horizontal', margin: 'sm', contents: [
            { type: 'text', text: '高溫 80°C', size: 'sm', flex: 4, color: '#FFFFFF' },
            { type: 'text', text: '毛巾、床單、厚棉衣', size: 'sm', flex: 6, wrap: true, color: '#E0E0E0' }
          ]},
          { type: 'separator', color: '#1A1A1E', margin: 'lg' },
          { type: 'text', text: '💡 小技巧', size: 'sm', weight: 'bold', margin: 'md', color: '#E5B94C' },
          { type: 'text', text: '1. 衣物不要塞太滿，七分滿最佳\n2. 加烘衣球可減少20%烘乾時間\n3. 牛仔褲建議反面烘乾避免褪色', size: 'xs', wrap: true, margin: 'sm', color: '#E0E0E0' }
        ]
      }
    }
  };
  ...
  return {
    ...
    contents: {
      ...
      header: {
        type: 'box', layout: 'vertical', paddingAll: '20px',
        contents: [
          { type: 'text', text: '📖 第一次使用教學', color: '#E5B94C', weight: 'bold', size: 'lg' }
        ]
      },
      body: {
        type: 'box', layout: 'vertical', spacing: 'md', paddingAll: '20px',
        contents: [
          { type: 'text', text: '只要 4 步驟，超簡單！', size: 'md', weight: 'bold', color: '#FFFFFF' },
          { type: 'separator', color: '#1A1A1E', margin: 'lg' },
          {
            type: 'box', layout: 'horizontal', margin: 'lg', contents: [
              {
                ...
                contents: [
                  {
                    ...
                    backgroundColor: '#1A1A1E', cornerRadius: '16px', justifyContent: 'center', alignItems: 'center',
          ...
          },
          {
            type: 'box', layout: 'horizontal', margin: 'md', contents: [
              ...
              { type: 'box', layout: 'vertical', flex: 7, contents: [
                { type: 'text', text: '放入衣物', size: 'sm', weight: 'bold', color: '#FFFFFF' },
                { type: 'text', text: '到店後選機器，放入衣物並關好機門', size: 'xs', color: '#999999', wrap: true }
              ]}
            ]
          },
          {
            type: 'box', layout: 'horizontal', margin: 'md', contents: [
              ...
              { type: 'box', layout: 'vertical', flex: 7, contents: [
                { type: 'text', text: '選模式付款', size: 'sm', weight: 'bold', color: '#FFFFFF' },
                { type: 'text', text: '選擇洗衣模式，用 LINE Pay 或錢包付款', size: 'xs', color: '#999999', wrap: true }
              ]}
            ]
          },
          {
            type: 'box', layout: 'horizontal', margin: 'md', contents: [
              {
                ...
                contents: [
                  {
                    type: 'box', layout: 'vertical', width: '32px', height: '32px',
                    backgroundColor: '#1B5E20', cornerRadius: '16px', justifyContent: 'center', alignItems: 'center',
                    contents: [{ type: 'text', text: '✓', color: '#FFFFFF', size: 'sm', align: 'center', weight: 'bold' }]
                  }
                ]
              },
              { type: 'box', layout: 'vertical', flex: 7, contents: [
                { type: 'text', text: '等通知取衣', size: 'sm', weight: 'bold', color: '#FFFFFF' },
                { type: 'text', text: '洗好後 LINE 會通知你，回來取衣就好', size: 'xs', color: '#999999', wrap: true }
              ]}
            ]
          }
        ]
      ...
      footer: {
        type: 'box', layout: 'vertical', spacing: 'sm', paddingAll: '15px',
        contents: [
          { type: 'button', style: 'primary', color: '#E5B94C', action: { type: 'uri', label: '立即開始', uri: LIFF_WASH } }
        ]
      }
    }
  };
...
function buildStoreListCarousel() {
  ...
  const bubbles = stores.map((s, i) => ({
    ...
    header: {
      ...
      contents: [
        { type: 'text', text: `📍 ${s.name}`, color: '#E5B94C', weight: 'bold', size: 'md', wrap: true }
      ]
    },
    body: {
      type: 'box', layout: 'vertical', spacing: 'sm', paddingAll: '14px',
      contents: [
        { type: 'text', text: s.addr, size: 'sm', wrap: true, color: '#E0E0E0' },
        { type: 'text', text: '⏰ 24小時營業・全年無休', size: 'xs', color: '#999999', margin: 'md' }
      ]
    },
    footer: {
  ...
  return {
    ...
    contents: {
      ...
      header: {
        type: 'box', layout: 'vertical', paddingAll: '20px',
        contents: [
          { type: 'text', text: '🏅 會員等級制度', color: '#E5B94C', weight: 'bold', size: 'lg' }
        ]
      },
      body: {
        type: 'box', layout: 'vertical', spacing: 'md', paddingAll: '20px',
...
function buildLaundryTipsCarousel() {
  ...
  const bubbles = tips.map(t => ({
    ...
    header: {
      type: 'box', layout: 'vertical', paddingAll: '14px',
      contents: [
        { type: 'text', text: `${t.emoji} ${t.title}`, color: '#E5B94C', weight: 'bold', size: 'md', wrap: true }
      ]
    },
    body: {
      type: 'box', layout: 'vertical', spacing: 'sm', paddingAll: '14px',
      contents: t.items.map((item, i) => ({
        type: 'text', text: item, size: 'xs', wrap: true, color: i === t.items.length - 1 ? '#E5B94C' : '#E0E0E0',
...
function buildSuccessCasesCarousel() {
  ...
  const bubbles = cases.map(c => ({
    ...
    header: {
      ...
      contents: [
        { type: 'text', text: `⭐ ${c.name} — ${c.store}`, color: '#E5B94C', weight: 'bold', size: 'md', wrap: true },
        { type: 'text', text: c.period, color: '#999999', size: 'xs', margin: 'xs' }
      ]
    },
    body: {
      type: 'box', layout: 'vertical', spacing: 'md', paddingAll: '16px',
      contents: [
        { type: 'text', text: `"${c.quote}"`, size: 'sm', wrap: true, style: 'italic', color: '#E0E0E0' },
        { type: 'separator', color: '#1A1A1E', margin: 'lg' },
        ...c.stats.map(s => ({
          type: 'box', layout: 'horizontal', margin: 'sm',
          contents: [
            { type: 'text', text: s.label, size: 'sm', color: '#999999', flex: 5 },
            { type: 'text', text: s.value, size: 'sm', weight: 'bold', flex: 4, align: 'end', color: '#E5B94C' }
          ]
        }))
      ]
    },
  ...
  return {
    ...
    contents: {
      ...
      header: {
        type: 'box', layout: 'vertical', paddingAll: '20px',
        contents: [
          { type: 'text', text: '⚡ 傳統投幣 vs 雲管家智慧系統', color: '#E5B94C', weight: 'bold', size: 'md', wrap: true }
        ]
      },
      body: {
        ...
        contents: [
          ...
          { type: 'box', layout: 'horizontal', margin: 'none', contents: [
            { type: 'text', text: '項目', size: 'xs', weight: 'bold', flex: 3, color: '#999999' },
            { type: 'text', text: '傳統投幣', size: 'xs', weight: 'bold', flex: 3, align: 'center', color: '#999999' },
            { type: 'text', text: '雲管家', size: 'xs', weight: 'bold', flex: 3, align: 'center', color: '#E5B94C' }
          ]},
          { type: 'separator', color: '#1A1A1E', margin: 'sm' },
          // Rows
          { type: 'box', layout: 'horizontal', margin: 'md', contents: [
            { type: 'text', text: '付款方式', size: 'xs', flex: 3, color: '#FFFFFF' },
            { type: 'text', text: '僅投幣', size: 'xs', flex: 3, align: 'center', color: '#999999' },
            { type: 'text', text: '多元支付', size: 'xs', flex: 3, align: 'center', color: '#4CAF50', weight: 'bold' }
          ]},
          { type: 'box', layout: 'horizontal', margin: 'sm', contents: [
            { type: 'text', text: '管理方式', size: 'xs', flex: 3, color: '#FFFFFF' },
            { type: 'text', text: '天天到場', size: 'xs', flex: 3, align: 'center', color: '#999999' },
            { type: 'text', text: '遠端15分鐘', size: 'xs', flex: 3, align: 'center', color: '#4CAF50', weight: 'bold' }
          ]},
          { type: 'box', layout: 'horizontal', margin: 'sm', contents: [
            { type: 'text', text: '客單價', size: 'xs', flex: 3, color: '#FFFFFF' },
            { type: 'text', text: 'NT$40-50', size: 'xs', flex: 3, align: 'center', color: '#999999' },
            { type: 'text', text: 'NT$55-70', size: 'xs', flex: 3, align: 'center', color: '#4CAF50', weight: 'bold' }
          ]},
          { type: 'box', layout: 'horizontal', margin: 'sm', contents: [
            { type: 'text', text: '會員經營', size: 'xs', flex: 3, color: '#FFFFFF' },
            { type: 'text', text: '無', size: 'xs', flex: 3, align: 'center', color: '#FF6B6B' },
            { type: 'text', text: '完整CRM', size: 'xs', flex: 3, align: 'center', color: '#4CAF50', weight: 'bold' }
          ]},
          { type: 'box', layout: 'horizontal', margin: 'sm', contents: [
            { type: 'text', text: '設備監控', size: 'xs', flex: 3, color: '#FFFFFF' },
            { type: 'text', text: '人工巡店', size: 'xs', flex: 3, align: 'center', color: '#999999' },
            { type: 'text', text: 'IoT即時', size: 'xs', flex: 3, align: 'center', color: '#4CAF50', weight: 'bold' }
          ]},
          { type: 'box', layout: 'horizontal', margin: 'sm', contents: [
            { type: 'text', text: '數據分析', size: 'xs', flex: 3, color: '#FFFFFF' },
            { type: 'text', text: '手記帳', size: 'xs', flex: 3, align: 'center', color: '#999999' },
            { type: 'text', text: '自動報表', size: 'xs', flex: 3, align: 'center', color: '#4CAF50', weight: 'bold' }
          ]},
          { type: 'box', layout: 'horizontal', margin: 'sm', contents: [
            { type: 'text', text: '回購率', size: 'xs', flex: 3, color: '#FFFFFF' },
            { type: 'text', text: '~30%', size: 'xs', flex: 3, align: 'center', color: '#999999' },
            { type: 'text', text: '~65%', size: 'xs', flex: 3, align: 'center', color: '#4CAF50', weight: 'bold' }
          ]},
          { type: 'separator', color: '#1A1A1E', margin: 'lg' },
          { type: 'text', text: '💡 雲管家智慧系統平均提升營收 25-40%', size: 'xs', color: '#E5B94C', weight: 'bold', margin: 'md', wrap: true }
        ]
      },
      footer: {
        type: 'box', layout: 'vertical', spacing: 'sm', paddingAll: '15px',
...
function buildPlansCard() {
    ...
    header: {
      ...
      contents: [
        ...
        { type: 'box', layout: 'vertical', backgroundColor: BRAND_GOLD, cornerRadius: '4px', paddingAll: '4px', paddingStart: '8px', paddingEnd: '8px', flex: 0, contents: [
          { type: 'text', text: '最熱門', size: 'xs', weight: 'bold', color: '#050508', align: 'center' }
        ]}
      ]
    },
    body: {
  ...
  const customBubble = {
    ...
    body: {
      type: 'box', layout: 'vertical', spacing: 'md', paddingAll: '20px',
      contents: [
        { type: 'text', text: '依需求報價', size: 'xl', weight: 'bold', color: '#E5B94C' },
        { type: 'separator', color: '#1A1A1E', margin: 'lg' },
    ...
    messages: [{
      ...
      contents: {
        ...
        header: { type: 'box', layout: 'vertical', paddingAll: '20px', contents: [
          { type: 'text', text: '📘 創業避坑指南', color: '#E5B94C', weight: 'bold', size: 'lg' }
        ]},
        body: { type: 'box', layout: 'vertical', spacing: 'md', paddingAll: '20px', contents: [
          { type: 'text', text: '5 個開店前必看的關鍵指標：', size: 'md', weight: 'bold', wrap: true, color: '#FFFFFF' },
          { type: 'text', text: '1️⃣ 選址密度比 — 半徑500m住戶÷競業數≥800', size: 'sm', wrap: true, margin: 'md', color: '#E0E0E0' },
          { type: 'text', text: '2️⃣ 設備投報率 — 單機日均使用≥4次才健康', size: 'sm', wrap: true, color: '#E0E0E0' },
          { type: 'text', text: '3️⃣ 金流結構 — 行動支付客單價比投幣高18%', size: 'sm', wrap: true, color: '#E0E0E0' },
          { type: 'text', text: '4️⃣ 遠端管理 — 每天只需15分鐘管整間店', size: 'sm', wrap: true, color: '#E0E0E0' },
          { type: 'text', text: '5️⃣ 會員經營 — 有會員系統的店終身價值高2.3倍', size: 'sm', wrap: true, color: '#E0E0E0' },
          { type: 'separator', color: '#1A1A1E', margin: 'lg' },
          { type: 'text', text: '想進一步了解？回覆「獲利分析」領取投報率試算表', size: 'xs', wrap: true, color: '#999999', margin: 'md' }
        ]},
  ...
  '獲利分析': {
    ...
    messages: [{
      ...
      contents: {
        ...
        header: { type: 'box', layout: 'vertical', paddingAll: '20px', contents: [
          { type: 'text', text: '📊 獲利分析概覽', color: '#E5B94C', weight: 'bold', size: 'lg' }
        ]},
        body: { type: 'box', layout: 'vertical', spacing: 'md', paddingAll: '20px', contents: [
          { type: 'text', text: '以 6 台機器標準店為例：', size: 'md', weight: 'bold', wrap: true, color: '#FFFFFF' },
          { type: 'box', layout: 'horizontal', margin: 'lg', contents: [
            { type: 'text', text: '月均營收', size: 'sm', color: '#999999', flex: 4 },
            { type: 'text', text: 'NT$ 8-15 萬', size: 'sm', weight: 'bold', flex: 6, align: 'end', color: '#FFFFFF' }
          ]},
          { type: 'box', layout: 'horizontal', contents: [
            { type: 'text', text: '月淨利', size: 'sm', color: '#999999', flex: 4 },
            { type: 'text', text: 'NT$ 4-8 萬', size: 'sm', weight: 'bold', flex: 6, align: 'end', color: '#FFFFFF' }
          ]},
          { type: 'box', layout: 'horizontal', contents: [
            { type: 'text', text: '預估回收期', size: 'sm', color: '#999999', flex: 4 },
            { type: 'text', text: '14-20 個月', size: 'sm', weight: 'bold', flex: 6, align: 'end', color: '#FFFFFF' }
          ]},
          { type: 'separator', color: '#1A1A1E', margin: 'lg' },
          { type: 'text', text: '雲管家加盟優勢：\n📊 數據分析精準投放\n🎯 會員系統拉高回購\n🔧 遠端管理省人力', size: 'sm', wrap: true, margin: 'md', color: '#E0E0E0' },
          { type: 'text', text: '想要客製化評估？回覆「區域評估」', size: 'xs', wrap: true, color: '#999999', margin: 'md' }
        ]},
        footer: { type: 'box', layout: 'vertical', spacing: 'sm', paddingAll: '15px', contents: [
          { type: 'button', style: 'primary', color: '#E5B94C', action: { type: 'uri', label: '預約免費 Demo', uri: LINE_OA_CHAT_URL } }
        ]}
      }
    }]
  },
  ...
  '故障報修': {
    ...
    messages: [{
      ...
      contents: {
        ...
        header: { type: 'box', layout: 'vertical', paddingAll: '20px', contents: [
          { type: 'text', text: '🔧 故障報修', color: '#E5B94C', weight: 'bold', size: 'lg' }
        ]},
        body: { type: 'box', layout: 'vertical', spacing: 'md', paddingAll: '20px', contents: [
          { type: 'text', text: '遇到機器問題了嗎？請提供以下資訊，我們會盡快處理：', size: 'sm', wrap: true, color: '#FFFFFF' },
          { type: 'separator', color: '#1A1A1E', margin: 'lg' },
          { type: 'text', text: '請回覆以下格式：', size: 'sm', weight: 'bold', margin: 'lg', wrap: true, color: '#E5B94C' },
          { type: 'text', text: '1️⃣ 哪家門市？（例如：悠洗自助洗衣）\n2️⃣ 哪台機器？（例如：洗衣機2號）\n3️⃣ 什麼問題？（例如：無法啟動）\n4️⃣ 是否有投幣/付款？', size: 'sm', wrap: true, margin: 'sm', color: '#E0E0E0' },
          { type: 'separator', color: '#1A1A1E', margin: 'lg' },
          { type: 'text', text: '💡 常見快速排除：', size: 'sm', weight: 'bold', margin: 'md', wrap: true, color: '#E5B94C' },
          { type: 'text', text: '• 門沒關好 → 確認機門完全關閉再按啟動\n• 投幣沒反應 → 試試用 LINE Pay 付款\n• 洗到一半停了 → 可能斷電，等2分鐘會自動恢復', size: 'xs', wrap: true, color: '#999999', margin: 'sm' }
        ]},
        footer: { type: 'box', layout: 'vertical', spacing: 'sm', paddingAll: '15px', contents: [
          { type: 'button', style: 'primary', color: '#E5B94C', action: { type: 'uri', label: '直接聯繫客服', uri: LINE_OA_CHAT_URL } },
  ...
  return {
    ...
    contents: {
      ...
      body: {
        type: 'box', layout: 'vertical', spacing: 'md', paddingAll: '20px',
        contents: [
          { type: 'text', text: '想更了解你的需求，請問目前的狀態是？', size: 'md', wrap: true, weight: 'bold', color: '#FFFFFF' }
        ]
      },
      footer: {
        type: 'box', layout: 'vertical', spacing: 'md', paddingAll: '15px',
        contents: [
          { type: 'button', style: 'primary', color: '#555555', height: 'sm', action: { type: 'postback', label: '🅰️ 還在觀望，想多了解', data: 'action=franchise_observe', displayText: '還在觀望，想多了解' } },
          ...
          { type: 'button', style: 'primary', color: '#E5B94C', height: 'sm', action: { type: 'postback', label: '🅾️ 已有洗衣店，想升級系統', data: 'action=franchise_upgrade', displayText: '已有洗衣店，想升級系統' } }
        ]
      }
    }
  };
  ...
  switch (action) {
    ...
    case 'start_survey': {
      ...
      await lineReply(event.replyToken, [{
        ...
        contents: {
          type: 'bubble',
          styles: { body: { backgroundColor: '#050508' } },
          body: {
            type: 'box', layout: 'vertical', paddingAll: '20px', spacing: 'md',
            contents: [
    ...
    case 'survey_answer': {
      ...
      if (nextStep === 'timeline' && answerStep !== 'timeline') {
        await lineReply(event.replyToken, [{
          ...
          contents: {
            type: 'bubble',
            styles: { body: { backgroundColor: '#050508' }, footer: { backgroundColor: '#050508', separator: false } },
            body: { type: 'box', layout: 'vertical', paddingAll: '16px', contents: [{ type: 'text', text: '⏱ 預計何時要上線？', color: '#E5B94C', weight: 'bold', size: 'md' }] },
            footer: {
              type: 'box', layout: 'vertical', paddingAll: '16px', spacing: 'sm',
              contents: [
                { type: 'button', style: 'primary', color: '#E5B94C', height: 'sm', action: { type: 'postback', label: '1 個月內', data: 'action=survey_answer&step=timeline&value=1month', displayText: '1個月內' } },
                { type: 'button', style: 'primary', color: '#E5B94C', height: 'sm', action: { type: 'postback', label: '1-3 個月', data: 'action=survey_answer&step=timeline&value=1-3months', displayText: '1-3個月' } },
                { type: 'button', style: 'primary', color: '#E5B94C', height: 'sm', action: { type: 'postback', label: '3 個月以上', data: 'action=survey_answer&step=timeline&value=3months+', displayText: '3個月以上' } },
    ...
    if (surveyProgress.rows.length > 0) {
      ...
      if (nextStep === 'done') {
        ...
        await sendLinePush(SUPER_ADMIN_ID, [{
          ...
          contents: {
            type: 'bubble',
            styles: { header: { backgroundColor: '#0A0A0F' }, body: { backgroundColor: '#050508' } },
            header: {
              type: 'box', layout: 'vertical', paddingAll: '16px',
        ...
        await lineReply(event.replyToken, [{
          ...
          contents: {
            ...
            footer: {
              type: 'box', layout: 'vertical', paddingAll: '16px', spacing: 'sm',
              contents: [
                { type: 'button', style: 'primary', color: '#E5B94C', height: 'sm', action: { type: 'postback', label: '月租 NT$5,000/月', data: 'action=survey_answer&step=plan&value=monthly', displayText: '月租方案' } },
                { type: 'button', style: 'primary', color: '#E5B94C', height: 'sm', action: { type: 'postback', label: '年租 NT$50,000/年', data: 'action=survey_answer&step=plan&value=annual', displayText: '年租方案' } },
                { type: 'button', style: 'primary', color: '#E5B94C', height: 'sm', action: { type: 'postback', label: '客製化 / 買斷', data: 'action=survey_answer&step=plan&value=custom', displayText: '客製化方案' } },
        ...
        await lineReply(event.replyToken, [{
          ...
          contents: {
            ...
            footer: {
              type: 'box', layout: 'vertical', paddingAll: '16px', spacing: 'sm',
              contents: [
                { type: 'button', style: 'primary', color: '#E5B94C', height: 'sm', action: { type: 'postback', label: '有，我知道型號', data: 'action=survey_answer&step=has_protocol&value=yes_know', displayText: '有通訊協議' } },
                { type: 'button', style: 'primary', color: '#E5B94C', height: 'sm', action: { type: 'postback', label: '有，但不確定型號', data: 'action=survey_answer&step=has_protocol&value=yes_unknown', displayText: '有但不確定' } },
        ...
        await lineReply(event.replyToken, [{
          ...
          contents: {
            ...
            footer: {
              type: 'box', layout: 'vertical', paddingAll: '16px', spacing: 'sm',
              contents: [
                { type: 'button', style: 'primary', color: '#E5B94C', height: 'sm', action: { type: 'postback', label: '小型（3-5台）', data: 'action=survey_answer&step=store_size&value=small', displayText: '小型 3-5台' } },
                { type: 'button', style: 'primary', color: '#E5B94C', height: 'sm', action: { type: 'postback', label: '中型（6-10台）', data: 'action=survey_answer&step=store_size&value=medium', displayText: '中型 6-10台' } },
                { type: 'button', style: 'primary', color: '#E5B94C', height: 'sm', action: { type: 'postback', label: '大型（11台以上）', data: 'action=survey_answer&step=store_size&value=large', displayText: '大型 11台以上' } }
              ]
            }
          }
        }]);
  ...
  return {
    ...
    contents: {
      ...
      body: {
        type: 'box', layout: 'vertical', spacing: 'md', paddingAll: '20px',
        contents: [
          { type: 'text', text: title || '', weight: 'bold', size: 'lg', wrap: true, color: '#FFFFFF' },
          { type: 'text', text: description || '', size: 'sm', color: '#999999', wrap: true, margin: 'md' }
        ]
      },
      footer: {
        type: 'box', layout: 'vertical', spacing: 'sm', paddingAll: '15px',
        contents: [
          { type: 'button', style: 'primary', color: '#E5B94C',
